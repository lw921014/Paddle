...
template <typename T>
class CAllGatherOpECCLKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    VLOG(3) << "begin eccl allgather, parameter is: "
            << ", group is " << group << ", ring_id is " << ring_id
            << ", nranks is " << nranks;

    PADDLE_ENFORCE_ECCL_SUCCESS(platform::dynload::eccl_all_gather(
...
REGISTER_OP_NPU_KERNEL(c_allgather, ops::CAllGatherOpECCLKernel<int8_t>,
                       ops::CAllGatherOpECCLKernel<int>,
                       ops::CAllGatherOpECCLKernel<float>,
                       ops::CAllGatherOpECCLKernel<plat::float16>);
