...
class ECCLCommContext {
 public:
  ...
  ECCLComm* CreateECCLComm(PaddleEcclCommGroupIdType eccl_id, int nranks, int rank,
                           int dev_id, int ring_id);
  // a latter comm with the same dev_id and the same ring_id
  // will override the former
  ECCLComm* AssignECCLComm(PaddleEcclCommGroupIdType comm, int nranks, int rank, int dev_id,
                           int ring_id);
