...
class HCCLComm {
 public:
  ...
  virtual int rank() const = 0;
  virtual int device_id() const = 0;
  virtual EcclCommGroupIdType comm() const = 0;
...
class HCCLCommContext {
 public:
  ...
  HCCLComm* CreateHCCLComm(EcclCommGroupIdType hccl_id, int nranks, int rank,
                           int dev_id, int ring_id);
  // a latter comm with the same dev_id and the same ring_id
  // will override the former
  HCCLComm* AssignHCCLComm(EcclCommGroupIdType comm, int nranks, int rank, int dev_id,
                           int ring_id);
